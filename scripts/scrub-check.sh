...
# im oeffentlichen Repo waere genau das Leck, das dieses Gate verhindern soll.
...
if [ $# -gt 0 ]; then
  ...
  echo "Keine Begriffe. Uebergib sie als Argumente oder lege .scrub-terms an." >&2
  exit 2
fi
[ ${#begriffe[@]} -eq 0 ] && { echo "Begriffsliste ist leer." >&2; exit 2; }

# Metazeichen entschaerfen: Begriffe sind Namen, keine Regexe.
...
if [ "$treffer" -eq 0 ]; then
  echo "sauber — $(git ls-files | wc -l) Dateien gegen ${#begriffe[@]} Begriff(e) geprueft"
  exit 0
fi
echo
